...
static void try_bulk_dequeue_skb(struct Qdisc *q,
				 struct sk_buff *skb,
				 const struct netdev_queue *txq,
				 int *packets/*出参，出队的报文数*/, int budget)
{
...
static struct sk_buff *dequeue_skb(struct Qdisc *q, bool *validate,
				   int *packets/*成功出队的报文数*/, int budget)
{
	const struct netdev_queue *txq = q->dev_queue;
 ...
 *
 */
static inline bool qdisc_restart(struct Qdisc *q, int *packets/*出参，可以出多少包*/, int budget)
{
	...
	/* Dequeue packet */
	skb = dequeue_skb(q, &validate, packets/*出队一组报文,利用next串起来*/, budget);
	if (unlikely(!skb))
		/*队列为空，直接返回*/
		return false;
...
void __qdisc_run(struct Qdisc *q)
{
	...
	while (qdisc_restart(q, &packets/*出队的报文数*/, quota)) {
		quota -= packets;
		if (quota <= 0) {
			if (q->flags & TCQ_F_NOLOCK)
				set_bit(__QDISC_STATE_MISSED, &q->state);
