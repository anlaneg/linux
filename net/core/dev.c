...
int netif_set_alias(struct net_device *dev, const char *alias/*接口别名*/, size_t len)
{
    //设置或别更设备别名称
...
static int netif_receive_skb_internal(struct sk_buff *skb)
{
	...
	net_timestamp_check(READ_ONCE(net_hotdata.tstamp_prequeue)/*此值为0时不设置时间签*/, skb);
				  ...
				  int weight)
{
	...
	if (napi_get_threaded_config(dev, napi))
		if (napi_kthread_create(napi))/*针对这种设备采用线程poll*/
...
int netif_set_allmulti(struct net_device *dev, int inc, bool notify)
{
	...
	if (allmulti == 0) {
		...
		flags = old_flags | IFF_ALLMULTI;/*使网络设备支持组播*/
	}
...
int netif_change_tx_queue_len(struct net_device *dev, unsigned long new_len)
{
	...
	if (new_len != orig_len) {
		/*触发tx队列长度变更事件*/
		WRITE_ONCE(dev->tx_queue_len, new_len);/*设置网络设备tx队列长度*/
...
/*返回给定设备的mac地址*/
int netif_get_mac_address(struct sockaddr *sa/*出参，此设备对应的硬件地址*/, struct net *net, char *dev_name/*设备名称*/)
{
...
int netif_get_port_parent_id(struct net_device *dev,
			     struct netdev_phys_item_id *ppid/*父id号*/, bool recurse)
{
	const struct net_device_ops *ops = dev->netdev_ops;
...
int register_netdevice(struct net_device *dev)
{
	...
	if (!(dev->rtnl_link_ops && dev->rtnl_link_initializing))
		rtmsg_ifinfo(RTM_NEWLINK/*触发newlink事件*/, dev, ~0U, GFP_KERNEL, 0, NULL);
