...
static int nl80211_trigger_scan(struct sk_buff *skb, struct genl_info *info)
{
	...
	if (scan_freqs) {
		...
		nla_for_each_nested(attr, scan_freqs, tmp) {
			...
			request->req.channels[i] = chan;/*设置请求的channel*/
			i++;
		}
	} else {
		enum nl80211_band band;
	...
	request->req.n_channels = i;/*更新实际填充的channel数*/
	...
	request->req.scan_start = jiffies;/*扫描起始时间*/
...
static const struct genl_small_ops nl80211_small_ops[] = {
	...
	},
	{
		...
		.validate = GENL_DONT_VALIDATE_STRICT | GENL_DONT_VALIDATE_DUMP,
		.doit = nl80211_new_station,/*添加station*/
		.flags = GENL_UNS_ADMIN_PERM,
		.internal_flags = IFLAGS(NL80211_FLAG_NEED_NETDEV_UP),
	},
	{
		...
		.validate = GENL_DONT_VALIDATE_STRICT | GENL_DONT_VALIDATE_DUMP,
		.doit = nl80211_del_station,/*删除station*/
		.flags = GENL_UNS_ADMIN_PERM,
