...
static struct nft_table *nft_table_lookup(const struct net *net,
					  ...
					  u8 family, u8 genmask/*下一代生效的mask*/, u32 nlpid)
{
	struct nftables_pernet *nft_net;
	struct nft_table *table;
...
{
	struct netlink_ext_ack *extack = info->extack;
	u8 genmask = nft_genmask_next(info->net);/*下一代的mask*/
	u8 family = info->nfmsg->nfgen_family;
	struct net *net = info->net;
	const struct nlattr *attr;
	struct nft_table *table;
	...
	nft_ctx_init(&ctx, net, skb, info->nlh, 0/*AF_UNSPEC取值*/, NULL/*table为空*/, NULL/*chain为空*/, nla);
	if (family == AF_UNSPEC ||
...
static int nft_chain_parse_hook(struct net *net,
				...
				struct netlink_ext_ack *extack, bool autoload/*是否自动加载chain type模块*/)
{
	struct nftables_pernet *nft_net = nft_pernet(net);
			     ...
			     struct nft_expr *expr/*出参，待初始化的表达式*/)
{
	...
	expr->ops = ops;
	if (ops->init) {
		err = ops->init(ctx, expr/*待初始化表达式*/, (const struct nlattr **)expr_info->tb/*表达式配置数组*/);
		if (err < 0)
			goto err1;
	}
				      ...
				      const struct nlattr *nla)
{
	...
	err = nf_tables_newexpr(ctx, &expr_info, expr/*待初始化的表达式*/);
	if (err < 0)
		goto err_expr_new;
	...
	if (nla[NFTA_RULE_EXPRESSIONS]) {
		...
		nla_for_each_nested(tmp, nla[NFTA_RULE_EXPRESSIONS], rem) {
			...
			if (n == NFT_RULE_MAXEXPRS)
				goto err_release_expr;
			//表达式解析
			err = nf_tables_expr_parse(&ctx, tmp, &expr_info[n]/*表达式对应的ops及数据*/);
			if (err < 0) {
...
static const struct nfnl_callback nf_tables_cb[NFT_MSG_MAX] = {
	[NFT_MSG_NEWTABLE] = {
		.call		= nf_tables_newtable,//netfilter table创建
		.type		= NFNL_CB_BATCH,
	...
	[NFT_MSG_NEWCHAIN] = {
		.call		= nf_tables_newchain,//新建chain
		.type		= NFNL_CB_BATCH,
	...
	[NFT_MSG_NEWSETELEM] = {
		.call		= nf_tables_newsetelem,//set的elem添加
		.type		= NFNL_CB_BATCH,
	...
	[NFT_MSG_NEWOBJ] = {
		.call		= nf_tables_newobj,//创建新的object
		.type		= NFNL_CB_BATCH,
	...
	[NFT_MSG_NEWFLOWTABLE] = {
		.call		= nf_tables_newflowtable,//添加flow table
		.type		= NFNL_CB_BATCH,
...
static int nft_value_init(const struct nft_ctx *ctx,
			  struct nft_data *data/*出参，value*/, struct nft_data_desc *desc/*出参，value的描述信息*/,
			  const struct nlattr *nla)
{
	unsigned int len;
