			 ...
			 int addr_len)
{
	...
	if (!err)
		err = vfs_mknod(idmap, d_inode(parent.dentry)/*父目录*/, dentry, mode, 0, NULL);
	if (err)
		goto out_path;
	err = mutex_lock_interruptible(&u->bindlock);
	if (err)
			       ...
			       size_t len)
{
	...
	while (sent < len) {
		...
		__skb_queue_tail(&other->sk_receive_queue, skb);/*内容入队*/
		spin_unlock(&other->sk_receive_queue.lock);

		unix_state_unlock(other);
		READ_ONCE(other->sk_data_ready)(other);/*通知收到数据*/
		sent += size;
	}
...
unlock:
	...
	if (msg) {
		...
		scm_recv_unix(sock, msg, &scm, flags);/*scm信息转msg*/
