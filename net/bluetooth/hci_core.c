	...
	case HCISETENCRYPT:
		...
		if (!test_bit(HCI_AUTH, &hdev->flags)) {
			...
			err = hci_cmd_sync_status(hdev,
						  ...
						  1, &dr.dev_opt,
						  HCI_CMD_TIMEOUT);/*先开启auth*/
			if (err)
				break;
		}

		err = hci_cmd_sync_status(hdev, HCI_OP_WRITE_ENCRYPT_MODE,
					  1, &dr.dev_opt, HCI_CMD_TIMEOUT);/*再开启encrypt*/
		break;
...
int hci_get_dev_list(void __user *arg)
{
	...
	err = copy_to_user(arg, dl, struct_size(dl, dev_req, n));/*将内容填充到arg指出的地址中*/
	kfree(dl);
...
static void hci_cmd_work(struct work_struct *work)
{
	...
	if (atomic_read(&hdev->cmd_cnt)) {
		...
		err = hci_send_cmd_sync(hdev, skb);/*同步发送此cmd给HCI硬件*/
		if (err)
			return;
