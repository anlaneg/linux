...
static struct vfsmount *shm_mnt __ro_after_init;/*tmpfs文件系统对应的全局挂载信息*/
...
repeat:
	...
	*foliop = NULL;
	...
	folio = shmem_alloc_and_add_folio(vmf, gfp, inode, index, fault_mm, 0/*非大页*/);
	if (IS_ERR(folio)) {
		error = PTR_ERR(folio);
		if (error == -EEXIST)
			goto repeat;
...
static struct inode *__shmem_get_inode(struct mnt_idmap *idmap,
				       struct super_block *sb,
				       struct inode *dir, umode_t mode/*包含文件类型*/,
				       dev_t dev, vma_flags_t flags)
{
	...
	info = SHMEM_I(inode);
	/*初始化info除vfs_inode以外字段为0*/
	memset(info, 0, (char *)inode - (char *)info);
	spin_lock_init(&info->lock);
	atomic_set(&info->stop_eviction, 0);
	info->seals = F_SEAL_SEAL;
	info->flags = vma_flags_test(&flags, VMA_NORESERVE_BIT)/*是否有no reserve标记*/
		? SHMEM_F_NORESERVE : 0;
...
static ssize_t shmem_file_read_iter(struct kiocb *iocb, struct iov_iter *to)
{
	...
	for (;;) {
		...
		index = iocb->ki_pos >> PAGE_SHIFT;/*起始位置所在的页*/
...
#ifdef CONFIG_TMPFS
	.parse_monolithic	= shmem_parse_monolithic,/*参数解析入口函数，调parse_param*/
...
static struct file *__shmem_file_setup(struct vfsmount *mnt, const char *name,
				       loff_t size/*文件大小*/, vma_flags_t flags,
 ...
 * @flags: VMA_NORESERVE_BIT suppresses pre-accounting of the entire object size
 */
struct file *shmem_file_setup(const char *name/*文件名*/, loff_t size, vma_flags_t flags)
{
	/*创建shmem_file*/
