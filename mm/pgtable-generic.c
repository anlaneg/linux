...
pte_t *__pte_offset_map(pmd_t *pmd, unsigned long addr, pmd_t *pmdvalp/*出参，用于保存pmd中保存的值*/)
{
	unsigned long irqflags;
	pmd_t pmdval;
	...
	if (pmdvalp)
		*pmdvalp = pmdval;/*记录pmd存的值*/
	if (unlikely(pmd_none(pmdval) /*pmd未填充*/|| !pmd_present(pmdval)))
		goto nomap;
	...
	return __pte_map(&pmdval, addr);/*利用pmd获得此addr对应的pte*/
nomap:
	rcu_read_unlock();
	return NULL;
}
			   ...
			   unsigned long addr, spinlock_t **ptlp)
{
	...
	pte_t *pte;
again:
	pte = __pte_offset_map(pmd, addr, &pmdval);/*取得pte*/
	if (unlikely(!pte))
		return pte;
	ptl = pte_lockptr(mm, &pmdval);
	spin_lock(ptl);/*对pmd加锁*/
