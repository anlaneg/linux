				    ...
				    unsigned long *page_mask)
{
	...
	pmd = pmd_offset(pudp, address);/*利用pud，针对address查找pmd*/
	...
	if (pmd_trans_huge(pmdval) && (flags & FOLL_SPLIT_PMD)) {
		...
		return pte_alloc(mm, pmd) ? ERR_PTR(-ENOMEM) :
			follow_page_pte(vma, address, pmd, flags);/*pte不需要则申请，否则取得pte*/
	}
				    ...
				    unsigned long *page_mask)
{
	...
	pudp = pud_offset(p4dp, address);/*利用p4d取地址对应的pud*/
	pud = pudp_get(pudp);
	if (!pud_present(pud))
		/*pud不存在情况*/
		return no_page_table(vma, flags, address);
				    ...
				    unsigned long *page_mask)
{
	...
	p4dp = p4d_offset(pgdp, address);/*利用pgd取p4d*/
	p4d = p4dp_get(p4dp);
	BUILD_BUG_ON(p4d_leaf(p4d));

	if (!p4d_present(p4d) || p4d_bad(p4d))
		/*p4d不存在或者p4d bad（address对应的地址还未映射)*/
		return no_page_table(vma, flags, address);
	 ...
	 unsigned long addr)
{
	...
	if (!vma || (addr >= vma->vm_start))
		/*利用addr查vma,如果没有查找返回；如果查找到了，且addr大于vma->start也返回查找结果*/
		return vma;

	/* Only warn for half-way relevant accesses */
	if (!(vma->vm_flags & VM_GROWSDOWN))
		return NULL;/*找到了vma且addr<vma->vm_start,但这个vma并不向下生长，返回NULL*/
	if (vma->vm_start - addr > 65536)
		return NULL;/*向下生长，但两者距离过大，返回NULL*/
...
static long __get_user_pages(struct mm_struct *mm,
		...
		unsigned int gup_flags, struct page **pages/*出参，填充由start开始的nr_pages个页*/,
		int *locked/*是否已加锁*/)
{
	long ret = 0, i = 0;
...
next_page:
		...
		i += page_increm;/*填充数增加*/
		start += page_increm * PAGE_SIZE;/*起始地址变更*/
		nr_pages -= page_increm;/*总需要页数减少*/
	} while (nr_pages);
out:
	return i ? i : ret;
}
...
static __always_inline long __get_user_pages_locked(struct mm_struct *mm,
						...
						struct page **pages/*出参，填充由start开始的nr_pages个页*/,
						int *locked/*入参，是否已加锁；出参，如未加锁，则指明已加锁*/,
						unsigned int flags)
{
	long ret, pages_done;
	...
	if (pages && !(flags & FOLL_PIN))
		/*提供了pages指针，对于无POLL_PIN标记的，为其补POLL_GET标记*/
		flags |= FOLL_GET;

	pages_done = 0;
	for (;;) {
		/*收集自start起始的nr_pages个页面指针，将其填充到pages中，ret是实际填充的pages总数*/
...
retry:
	...
	if (must_unlock && *locked) {
		 ...
		 */
		mmap_read_unlock(mm);/*指明必须解锁，且已加锁，则解锁*/
		*locked = 0;
	}
				   ...
				   struct pages_or_folios *pofs)
{
	...
	for (i = 0; i < pofs->nr_entries; i++) {
		...
		if (folio_is_device_coherent(folio)) {
			...
			folio_get(folio);
			gup_put_folio(folio, 1, FOLL_PIN);/*pin住内存页*/
...
static bool is_valid_gup_args(struct page **pages, int *locked,
			      unsigned int *gup_flags_p/*入出参*/, unsigned int to_set/*要增加的标记*/)
{
 ...
 * pins in it and unpin_user_page*() will not remove pins from it.
 */
long pin_user_pages_remote(struct mm_struct *mm/*remote进程关联的mm*/,
			   unsigned long start/*起始地址*/, unsigned long nr_pages/*总页数*/,
			   unsigned int gup_flags, struct page **pages/*出参，填充由start开始的nr_pages个页*/,
			   int *locked/*是否已加锁*/)
{
	...
	return __gup_longterm_locked(mm, start, nr_pages, pages,
				     locked ? locked : &local_locked/*指明已加锁*/,
				     gup_flags);
}
 ...
 * pins in it and unpin_user_page*() will not remove pins from it.
 */
long pin_user_pages(unsigned long start/*起始地址*/, unsigned long nr_pages/*总页数*/,
		    unsigned int gup_flags, struct page **pages)
{
	int locked = 1;

	if (!is_valid_gup_args(pages, NULL, &gup_flags, FOLL_PIN))
		return 0;/*gup_flags标记有误，返回0*/
	return __gup_longterm_locked(current->mm/*当前进程的mm*/, start, nr_pages,
