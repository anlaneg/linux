 ...
 */
int vmap_pages_range(unsigned long addr/*起始地址*/, unsigned long end/*终止地址*/,
		pgprot_t prot, struct page **pages, unsigned int page_shift)
{
	int err;
...
static __cacheline_aligned_in_smp atomic_long_t nr_vmalloc_pages;/*记录系统中通过vmalloc申请到的总页数*/
		...
		unsigned int order/*单页大小*/, unsigned int nr_pages/*pages指针大小（要申请的总页数）*/, struct page **pages/*出参，申请到的物理页*/)
{
	...
	while (nr_allocated < nr_pages) {
		...
		if (nid == NUMA_NO_NODE)
			page = alloc_pages_noprof(gfp, order);/*申请物理页*/
		else
 ...
 * Return: the address of the area or %NULL on failure
 */
void *__vmalloc_node_range_noprof(unsigned long size/*要申请的内存大小*/, unsigned long align/*对齐方式*/,
...
{
	return __vmalloc_node_noprof(size, 1/*按1字节对齐*/, gfp_mask, NUMA_NO_NODE/*不关注numa node*/,
				__builtin_return_address(0));
}
