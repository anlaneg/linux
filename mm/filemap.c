		...
		struct mempolicy *policy)
{
	...
	if (cpuset_do_page_mem_spread()) {
		...
		do {
			...
			folio = __folio_alloc_node_noprof(gfp, order, n);/*申请order大小的物理内存*/
		} while (!folio && read_mems_allowed_retry(cpuset_mems_cookie));

		return folio;
	}
	return folio_alloc_noprof(gfp, order);/*申请FOLIO*/
}
...
no_page:
	if (!folio && (fgp_flags & FGP_CREAT)) {
		...
		if (order > mapping_max_folio_order(mapping))
			order = mapping_max_folio_order(mapping);/*申请的order过大*/
		...
		do {
			...
			folio_put(folio);
			folio = NULL;
		} while (order-- > min_order);/*减少order,如果仍大于min_order,则继续申请*/
...
static int filemap_create_folio(struct kiocb *iocb, struct folio_batch *fbatch)
{
	...
	folio = filemap_alloc_folio(mapping_gfp_mask(mapping), min_order/*申请min_order页*/, NULL);
	if (!folio)
		return -ENOMEM;
...
repeat:
	...
	if (IS_ERR(folio)) {
		...
		folio = filemap_alloc_folio(gfp, mapping_min_folio_order(mapping), NULL);/*申请min_order页,但按folio初始化*/
		if (!folio)
...
retry:
		offset = pos & (chunk - 1);/*获取写入位置当前在具体一页中的偏移量*/
		bytes = min(chunk - offset, bytes);/*获取在首页的写入长度*/
		...
		status = a_ops->write_begin(iocb, mapping, pos/*写入的位置*/, bytes/*写入的长度*/,
						&folio/*出参，写入位置对应的页*/, &fsdata);
