		...
		void *pool_data, gfp_t gfp_mask, int node_id)
{
	 ...
	 */
	while (pool->curr_nr < max(1, pool->min_nr)) {/*初始化pool->min_nr个元素*/
		void *element;
...
struct mempool *mempool_create_node_noprof(int min_nr,
		mempool_alloc_t *alloc_fn, mempool_free_t *free_fn,
		void *pool_data, gfp_t gfp_mask, int node_id/*mempool关联的numa node*/)
{
	struct mempool *pool;
 ...
 * Return: %0 on success, negative error code otherwise.
 */
int mempool_resize(struct mempool *pool, int new_min_nr/*新的min_nr*/)
{
	void *element;
	void **new_elements;
	unsigned long flags;
