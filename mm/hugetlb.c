...
int hugetlb_vma_lock_alloc(struct vm_area_struct *vma)
{
	...
	if (vma->vm_private_data)
		return -EINVAL;/*已有private_data,直接返回*/
...
retry:
	...
	for (i = 0; i < needed; i++) {
		...
		folio = alloc_surplus_hugetlb_folio(h, htlb_alloc_mask(h),
						    NUMA_NO_NODE/*不关注numa*/, &alloc_nodemask);
		if (!folio) {
			alloc_ok = false;
			break;
		}
...
hugetlb_early_param("hugepagesz", hugepagesz_setup);/*解析hugepagesz配置*/
...
__setup("hugepage_alloc_threads=", hugepage_alloc_threads_setup);/*处理默认hugepage size的配置*/
...
/*实现大页内存预留*/
long hugetlb_reserve_pages(struct inode *inode/*关联的inode*/,
		long from/*大页起始序号*/, long to/*大页终止序号*/,
		struct vm_area_desc *desc/*关联的vma,可以为空*/,
