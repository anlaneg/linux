...
struct rxe_bth {
	u8			opcode;/*操作码*/
	/*分别对应SE标记(1bit)，MIG标记(1bit)，PAD（2bit)，tver(4bits)*/
	u8			flags;
	/*
...
/*
 * 发送方在报文中添加此标记，当收到此标记后，接收方需要生成一个CQE事件给上层
...
/*按参数se设置bth头部的SE标记，这个标记将促使接收方生成一个CQE事件*/
...
/*填写bth头*/
static inline void bth_init(struct rxe_pkt_info *pkt, u8 opcode, int se/*是否设置se标记，促使接收方产生CQE*/,
			    int mig/*是否有mig标记，标记应用/虚机正在迁移*/, int pad/*pad长度*/, u16 pkey/*对应的pkey*/, u32 qpn/*关联的24位有效的qpn*/, int ack_req/*是否要求回复ack*/,
			    u32 psn)
{
...
/*扩展头*/
struct rxe_rdeth {
	__be32			een;/*24位有效，高8位为预留位(9.3.1)，
	This field indicates which End-to-End Context should be used for this Reliable Datagram packet.
	*/
};
 ...
 ******************************************************************************/
struct rxe_aeth {
	/*AETH头出现在ACK报文，由两部分组成1。Syndrome;2.MSN*/
	__be32			smsn;
};
