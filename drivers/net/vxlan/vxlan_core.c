					...
					__be32 vni)
{
	...
	if (likely(f)) {
		...
		if (likely(vxlan_addr_equal(&rdst->remote_ip, src_ip) &&
			...
			return SKB_NOT_DROPPED_YET;//不需要更新
					  ...
					  struct sk_buff *skb, __be32 vni/*报文中对应的vxlan*/)
{
	...
	return vxlan_snoop(skb->dev, &saddr/*外层ip*/, eth_hdr(skb)->h_source/*内层源mac*/,
			   ifindex/*入接口*/, vni/*报文指定的vxlan id*/);
}
...
void vxlan_xmit_one(struct sk_buff *skb, struct net_device *dev,
		    __be32 default_vni, struct vxlan_rdst *rdst/*vxlan对端*/, bool did_rsc)
{
	...
	if (addr_family == AF_INET) {
		...
		//查路由确定下一跳
		rt = udp_tunnel_dst_lookup(skb, dev, vxlan->net, ifindex/*出接口*/,
					   &saddr/*隧道对端目的地址*/, pkey, src_port, dst_port,
