...
static void *vring_alloc_queue(struct virtio_device *vdev, size_t size/*队列内存大小*/,
			       dma_addr_t *map_handle/*出参，dma地址*/, gfp_t flag,
			       union virtio_map map)
{
	if (vring_use_map_api(vdev)) {
...
static int vring_alloc_queue_packed(struct vring_virtqueue_packed *vring_packed,
				    struct virtio_device *vdev,
				    u32 num/*队列深度*/, union virtio_map map)
{
...
#endif
	vq->map = map;
	vq->use_map_api = vring_use_map_api(vdev);/*命名用dma api，则需要执行unmap*/
...
int virtqueue_add_outbuf(struct virtqueue *vq,
			 ...
			 gfp_t gfp)
{
	return virtqueue_add(vq, &sg, num, 1/*指明为发送的SG*/, 0, data, NULL, false, gfp, 0);
}
...
int virtqueue_add_inbuf(struct virtqueue *vq,
			...
			gfp_t gfp)
{
	return virtqueue_add(vq, &sg, num, 0, 1/*指明为接收的SG*/, data, NULL, false, gfp, 0);
}
...
int virtqueue_add_inbuf_ctx(struct virtqueue *vq,
			...
			gfp_t gfp)
{
	return virtqueue_add(vq, &sg, num, 0, 1/*指明为接收的SG*/, data, ctx, false, gfp, 0);
}
...
static void vring_free(struct virtqueue *_vq)
{
	...
	if (vq->we_own_ring) {
		...
		if (virtqueue_is_packed(vq)/*packed类型的vring*/) {
			/*移除desc表*/
...
unsigned int virtqueue_get_vring_size(const struct virtqueue *_vq)
{
	...
	return virtqueue_is_packed(vq) ? vq->packed.vring.num/*packed队列长度*/ :
				      vq->split.vring.num;
}
