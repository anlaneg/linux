			  ...
			  int (*open)(struct inode *, struct file *))
{
	...
	struct inode *inode = f->f_path.dentry->d_inode;/*文件对应的inode*/
	int error;
	...
	if ((f->f_mode & FMODE_WRITE) &&
		...
		//如有write或write_iter回调，才能置write标记
...
static int do_sys_openat2(int dfd, const char __user *filename/*文件名称或文件路径*/,
			  struct open_how *how)
{
	struct open_flags op;
	struct filename *tmp;
