...
static struct inode *find_inode(struct super_block *sb/*inode所属super block*/,
				...
				void *data/*匹配函数参数*/, bool hash_locked,
				bool *isnew)
{
...
again:
	 ...
	 */
	spin_lock(&inode->i_lock);
	inode_state_set(inode, I_NEW);/*标记此inode为新建*/
	/*将此inode加入到桶上*/
...
again:
	...
	if (inode) {
		...
		if (!old) {
			...
			inode->i_ino = ino;
			spin_lock(&inode->i_lock);
			inode_state_assign(inode, I_NEW);/*指明此inode为new(需要新建）*/
...
struct inode *ilookup5_nowait(struct super_block *sb/*inode所属super block*/, unsigned long hashval,
		int (*test)(struct inode *, void *)/*匹配检测函数*/, void *data/*匹配检测函数的2号参数*/, bool *isnew)
{
