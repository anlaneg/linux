...
do_getname(const char __user *filename/*路径名称*/, int flags, bool incomplete)
{
...
static noinline struct dentry *lookup_slow(const struct qstr *name/*要查询的文件或目录名称*/,
				  struct dentry *dir/*父dentry节点*/,
				  unsigned int flags)
{
	struct inode *inode = dir->d_inode;
...
int filename_lookup(int dfd/*位置信息*/, struct filename *name/*路径名称*/, unsigned flags,
		    struct path *path/*出参，确定路径对应的dentry*/, const struct path *root/*文件系统根节点对应的path*/)
{
	int retval;
	struct nameidata nd;
	if (IS_ERR(name))
...
int kern_path(const char *name/*要解析的文件路径*/, unsigned int flags, struct path *path/*出参，name对应的path信息*/)
{
	...
	return filename_lookup(AT_FDCWD, filename, flags, path, NULL/*不指定root path*/);
}
		 ...
		 struct path *path/*出参，确定路径对应的dentry*/)
{
	...
	return filename_lookup(dfd, filename, flags, path, NULL/*不提供root path*/);
}
 ...
 * raw inode simply pass @nop_mnt_idmap.
 */
int vfs_create(struct mnt_idmap *idmap, struct dentry *dentry/*对应的dentry*/, umode_t mode,
	       struct delegated_inode *di)
{
	...
	if (error)
		return error;
	/*调用fs回调，完成普通文件创建*/
	error = dir->i_op->create(idmap, dir, dentry, mode/*文件mode*/, true);
	if (!error)
...
struct file *do_file_open(int dfd, struct filename *pathname/*文件路径*/,
		const struct open_flags *op)
{
	struct nameidata nd;
...
struct dentry *start_creating_path(int dfd/*锚点*/, const char *pathname,
...
int filename_mkdirat(int dfd/*patchname为相对地址时的锚点*/, struct filename *name/*要创建的目录路径*/, umode_t mode)
{
	struct dentry *dentry;
	struct path path;
	int error;
...
retry:
	...
	if (!error) {
		dentry = vfs_mkdir(mnt_idmap(path.mnt), path.dentry->d_inode/*父目录对应的inode*/,
				   dentry/*父目录对应的dentry*/, mode, &delegated_inode);
		if (IS_ERR(dentry))
	      ...
	      struct dentry *dentry, struct delegated_inode *delegated_inode)
{
	...
	error = try_break_deleg(dir/*父目录*/, delegated_inode/*要删除的dentry*/);
	if (error)
		goto out;
...
{
	CLASS(filename, name)(pathname);
	return filename_unlinkat(AT_FDCWD/*针对当前工作目录*/, name);
}
...
int vfs_rename(struct renamedata *rd)
{
	...
	error = old_dir->i_op->rename(rd->mnt_idmap, old_dir/*旧的父目录*/, old_dentry/*旧的dentry*/,
