...
static int hugetlbfs_file_mmap_prepare(struct vm_area_desc *desc)
{
	 ...
	 * and ia64).
	 */
	vma_desc_set_flags(desc, VMA_HUGETLB_BIT, VMA_DONTEXPAND_BIT);/*vma打hugetlb, do not expand标记*/
	...
	if (hugetlb_reserve_pages(inode,
			...
			len >> huge_page_shift(h)/*要映射的大页数目*/, desc,
			vma_flags) < 0)
		goto out;
...
static ssize_t hugetlbfs_read_iter(struct kiocb *iocb, struct iov_iter *to)
{
	...
	while (iov_iter_count(to)) {
		...
		/* Find the folio */
		folio = filemap_lock_hugetlb_folio(h, mapping, index);/*取index号对应的page*/
		if (IS_ERR(folio)) {
			/*
			 ...
			 */
			copied = copy_folio_to_iter(folio, offset, want, to);/*将页里的内容填充进buffer*/
			folio_put(folio);
		}
		offset += copied;
		retval += copied;
					...
					struct hugetlbfs_fs_context *ctx)
{
	...
	if (inode) {
		...
		inode->i_mode = S_IFDIR | ctx->mode;/*标明是目录*/
		inode->i_uid = ctx->uid;
		inode->i_gid = ctx->gid;
...
static struct inode *hugetlbfs_get_inode(struct super_block *sb,
					...
					struct inode *dir,
					umode_t mode/*inode类型权限*/, dev_t dev)
{
	struct inode *inode;
...
{
	return hugetlbfs_mknod(idmap, dir, dentry, mode | S_IFREG/*创建普通文件*/, 0);
}
...
static const struct file_operations hugetlbfs_file_operations = {
	...
	.mmap_prepare		= hugetlbfs_file_mmap_prepare,/*具体实现file到vma的映射*/
...
static int hugetlbfs_parse_param(struct fs_context *fc, struct fs_parameter *param)
{
	...
	/*利用hugetlb_fs_parameters描述的标准，解析param,结果填入result*/
...
static int hugetlbfs_validate(struct fs_context *fc)
{
	 ...
	 */
	if (ctx->max_val_type > NO_SIZE/*仅非no size时检查生效（未指定size时生效）*/ &&
...
static const struct fs_context_operations hugetlbfs_fs_context_ops = {
	...
	.parse_param	= hugetlbfs_parse_param/*huge提供的mount时data参数解析函数*/,
	.get_tree	= hugetlbfs_get_tree,/*用于获取root，设置fc->root*/
};
...
static int hugetlbfs_init_fs_context(struct fs_context *fc)
{
	...
	/*设置fs_context的私有数据为hugetlbfs_fs_context*/
	fc->fs_private = ctx;
	/*设置fs_context操作集*/
...
static struct vfsmount *__init mount_one_hugetlbfs(struct hstate *h)
{
	...
	/*为hugepage创建fs_context,指明sb_flags为kernel mount*/
