...
static void flock_make_lock(struct file *filp, struct file_lock *fl, int type)
{
	...
	fl->c.flc_flags = FL_FLOCK;/*指明FLOCK样式lock*/
	fl->c.flc_type = type;
...
new_blocker:
	...
	list_add_tail(&waiter->flc_blocked_member,
		      &blocker->flc_blocked_requests);/*将waiter加入等待*/
...
find_conflict:
	list_for_each_entry(fl, &ctx->flc_flock, c.flc_list) {
		...
		locks_insert_block(&fl->c, &request->c, flock_locks_conflict/*冲突检查函数*/);
		goto out;/*仅检查，不直实加锁*/
	}
	if (request->c.flc_flags & FL_ACCESS)
		goto out;
