...
anon_pipe_write(struct kiocb *iocb, struct iov_iter *from)
{
	...
	if (chars && !was_empty) {
		struct pipe_buffer *buf = pipe_buf(pipe, head - 1);/*当前写入位置对应的buffer*/
...
const struct file_operations pipefifo_fops = {
	.open		= fifo_open,
	.read_iter	= fifo_pipe_read,/*读端操作函数*/
	.write_iter	= fifo_pipe_write,/*写端操作函数*/
	.poll		= pipe_poll,
