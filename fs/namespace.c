...
mountpoint:
	...
	read_seqlock_excl(&mount_lock);
	mp->m_dentry = dget(dentry);/*记录挂载点对应的dentry*/
...
static void setup_mnt(struct mount *m, struct dentry *root)
{
	...
	m->mnt.mnt_sb = s;/*设置super block*/
	m->mnt.mnt_root = dget(root);/*设置挂载点对应的root dentry*/
	m->mnt_mountpoint = m->mnt.mnt_root;/*为什么要指两遍？*/
	m->mnt_parent = m;/*父挂载点为其自身*/
 ...
 */
static int do_add_mount(struct mount *newmnt/*要挂载的文件系统*/, const struct pinned_mountpoint *mp,
			int mnt_flags)
{
 ...
 */
static int do_new_mount_fc(struct fs_context *fc, const struct path *mountpoint/*设备挂载目标点*/,
			   unsigned int mnt_flags)
{
	...
	error = do_add_mount(real_mount(mnt)/*传入mount结构体*/, &mp, mnt_flags);
	if (!error)
 ...
 * namespace's tree
 */
static int do_new_mount(const struct path *path/*目标挂载点*/, const char *fstype/*文件系统名称*/,
			int sb_flags/*super block标记*/, int mnt_flags/*挂载标记*/,
			const char *name/*source参数取值,指明设备地址*/, void *data/*各fs自定义的数据*/)
{
 ...
 */
int path_mount(const char *dev_name/*设备名称*/, const struct path *path/*挂载点目录名称*/,
...
int do_mount(const char *dev_name/*设备名称*/, const char __user *dir_name/*挂载点目录名称*/,
		const char *type_page/*文件系统类型*/, unsigned long flags/*挂载flags*/, void *data_page/*文件系统私有选项*/)
{
	...
	if (ret)
		return ret;
	return path_mount(dev_name/*设备名称*/, &path/*挂载点路径*/, type_page/*文件系统类型*/, flags, data_page);
}
...
__latent_entropy
struct mnt_namespace *copy_mnt_ns(u64 flags, struct mnt_namespace *ns/*源mount namespace*/,
...
void __init mnt_init(void)
{
	...
	if (err)
		printk(KERN_WARNING "%s: sysfs_init error: %d\n",
			__func__, err);
	fs_kobj = kobject_create_and_add("fs", NULL);/*创建/sys/fs对应的kobject*/
	if (!fs_kobj)
