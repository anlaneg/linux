...
#ifdef CONFIG_NET
//1.BPF_PROG_TYPE 第二个参数 + _verifier_ops 为verifier对应的ops（存放在bpf_verifier_ops中）
//2.BPF_PROG_TYPE 第二个参数 + _prog_ops 为prog对应的ops(存放在bpf_prog_types中）
//3.BPF_MAP_TYPE  会在每个文件中定义其类型对应的_ops变量，这些变量会被集中在bpf_map_types数组中（存放在bpf_map_types中）
