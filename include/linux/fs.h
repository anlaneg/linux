...
/* file is seekable */
#define FMODE_LSEEK		((__force fmode_t)(1 << 2)) /*指明文件可以seek*/
/* file can be accessed using pread */
#define FMODE_PREAD		((__force fmode_t)(1 << 3)) /*指明文件可以pread*/
/* file can be accessed using pwrite */
#define FMODE_PWRITE		((__force fmode_t)(1 << 4)) /*指明文件可以pwrite*/
...
struct inode {
	...
	/* Misc */
	struct inode_state_flags i_state;/*inode状态，例如：I_NEW*/
	/* 32-bit hole */
...
{
	return inode->i_atime_sec;/*获取inode访问时间*/
}

static inline long inode_get_atime_nsec(const struct inode *inode)
{
	return inode->i_atime_nsec;/*获取inode访问时间*/
}
...
static inline struct timespec64 inode_get_atime(const struct inode *inode)
{
	...
	return ts;/*获取inode访问时间*/
}

static inline struct timespec64 inode_set_atime_to_ts(struct inode *inode,
						      struct timespec64 ts)
{
	inode->i_atime_sec = ts.tv_sec;/*设置inode访问时间*/
...
struct inode_operations {
	...
	struct dentry *(*mkdir) (struct mnt_idmap *, struct inode */*父目录inode*/,
				 struct dentry */*此目录对应的dentry*/, umode_t);
	/*删除目录inode*/
			    ...
			    u64 ino/*文件对应的inode*/, unsigned type)
{
	...
	return ctx->actor(ctx, name, namelen, ctx->pos/*当前偏移*/, ino, type & dt_mask);
}
