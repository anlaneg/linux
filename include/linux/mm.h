...
{
	vma_start_write(vma);
	vma_flags_set_word(&vma->flags, flags);/*设置flags*/
}
...
static inline unsigned int folio_order(const struct folio *folio)
{
	if (!folio_test_large(folio))
		return 0;
	return folio_large_order(folio);/*获得folio对应的order*/
}
...
static inline int put_page_testzero(struct page *page)
{
	...
	return page_ref_dec_and_test(page);/*put此页(减少引用计数)*/
}
				      ...
				      struct ptdesc *ptdesc)
{
	if (mm != &init_mm && !pmd_ptlock_init(ptdesc))
		return false;
	ptdesc_pmd_pts_init(ptdesc);/*打上PGTABLE标记*/
	__pagetable_ctor(ptdesc);
	return true;
}
