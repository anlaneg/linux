...
__alloc_pages_node_noprof(int nid/*申请内存优先选择的numa node*/, gfp_t gfp_mask/*标记位*/, unsigned int order/*内存大小的对数，log2(N)*/)
{
    //校验node id及mask
 ...
 * online.
 */
static inline struct page *alloc_pages_node_noprof(int nid/*内存对应的node*/, gfp_t gfp_mask/*标记位*/,
...
#define __get_free_page(gfp_mask)					\
	__get_free_pages((gfp_mask), 0/*一个空闲页*/)
