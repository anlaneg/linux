 ...
 * @flags: the type of memory to allocate (see kmalloc).
 */
#define kcalloc(n/*元素数目*/, size/*数组元素大小*/, flags)		kmalloc_array(n, size, (flags) | __GFP_ZERO)
...
#define kvmalloc_node(_s, _f, _n)		kvmalloc_node_align(_s/*要申请的内存大小*/, 1, _f, _n)
/*对于较小的内存，通过slab分配，对于较大内存通过alloc_pages直接分配*/
#define kvmalloc(...)				kvmalloc_node(__VA_ARGS__, NUMA_NO_NODE/*node不关心*/)
