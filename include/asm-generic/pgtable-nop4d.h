...
typedef struct { pgd_t pgd; } p4d_t;/*X86_64中此结构占用8个字节（此文件被包含时p4d不存在)*/
 ...
 * into the pgd entry)
 */
static inline int pgd_none(pgd_t pgd)		{ return 0; } /*直接返回0,表示PGD恒存在*/
static inline int pgd_bad(pgd_t pgd)		{ return 0; } /*直接返回0，表示未bad*/
